...
-- -- SELECT hot_nom, hot_ville FROM `hotel` WHERE 1; 
...
--SELECT hot_nom, hot_categorie, hot_ville, sta_nom FROM `hotel` INNER JOIN `station` ON sta_id = hot_sta_id; 
...
--SELECT hot_nom, hot_categorie, hot_ville, cha_numero FROM `chambre` INNER JOIN `hotel` ON cha_hot_id = hot_id; 
...
-- SELECT hot_nom, hot_categorie, hot_ville, cha_numero, cha_capacite FROM `chambre` INNER JOIN `hotel` ON chambre.cha_hot_id = hotel.hot_id WHERE cha_capacite > 1 AND hot_ville = "Bretou";
...
-- SELECT cli_nom, hot_nom, res_date FROM `reservation` INNER JOIN `client` ON res_cli_id = cli_id JOIN `chambre` ON res_cha_id = cha_id JOIN `hotel` ON cha_hot_id = hot_id; 
...
-- SELECT sta_nom, hot_nom, cha_numero, cha_capacite FROM `chambre` INNER JOIN `hotel` ON cha_hot_id = hot_id JOIN `station` ON hot_sta_id = sta_id; 
...
-- SELECT cli_nom, hot_nom, DATEDIFF(res_date_fin, res_date_debut) AS 'Duréé sejour', res_date_debut FROM `reservation` INNER JOIN `client` ON cli_id = res_cli_id JOIN `chambre` ON cha_id = res_cha_id JOIN `hotel` ON cha_hot_id = hot_id;
...
-- SELECT sta_nom, COUNT(hot_sta_id) AS "nombre d'hotel" FROM `hotel` INNER JOIN `station` ON hot_sta_id = sta_id GROUP BY sta_nom; 

-- -------------------------------------------------------
-- 14 - Compter le nombre de chambres par station

-- SELECT sta_nom, COUNT(cha_hot_id) AS "nombre de chambre" FROM `chambre` INNER JOIN `hotel` ON cha_hot_id = hot_id INNER JOIN `station`ON hot_sta_id = sta_id GROUP BY sta_nom;

-- -------------------------------------------------------
-- 15 - Compter le nombre de chambres par station ayant une capacité > 1

-- SELECT sta_nom, COUNT(cha_hot_id) AS "nombre de chambre" FROM `chambre` INNER JOIN `hotel` ON cha_hot_id = hot_id INNER JOIN `station`ON hot_sta_id = sta_id WHERE cha_capacite > 1 GROUP BY sta_nom;

-- -------------------------------------------------------
-- 16 - Afficher la liste des hôtels pour lesquels Mr Squire a effectué une réservation

-- SELECT hot_nom FROM `hotel` INNER JOIN `chambre` ON cha_hot_id = hot_id INNER JOIN `reservation` ON res_cha_id = cha_id INNER JOIN `client` ON cli_id = res_cli_id WHERE cli_nom = "Squire";

-- -------------------------------------------------------
-- 17 - Afficher la durée moyenne des réservations par station

--SELECT sta_nom, AVG(DATEDIFF(res_date_fin, res_date_debut)) AS "moy_reservation" FROM `station` INNER JOIN `hotel` ON sta_id = hot_sta_id INNER JOIN `chambre`ON hot_id = cha_hot_id INNER JOIN `reservation` ON res_cha_id = cha_id  